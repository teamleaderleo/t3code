...
class OpenCodeAdapter extends Context.Service<OpenCodeAdapter, OpenCodeAdapterShape>()(
  "t3/provider/Layers/OpenCodeAdapter.pending-request-stop.test/OpenCodeAdapter",
) {}

const sessionId = "ses_pending_request_stop";
const permissionId = ApprovalRequestId.make("per_pending_stop");
const questionId = ApprovalRequestId.make("que_pending_stop");
const EVENT_STREAM_END = Symbol("pending-request-stop/event-stream-end");
...
function makeHarness() {
  ...
  const runtime: OpenCodeRuntimeShape = {
    startOpenCodeServerProcess: () =>
      Effect.succeed({ url: "http://127.0.0.1:4302", exitCode: Effect.never }),
...
it.effect("expires pending requests before an explicit session stop is projected", () => {
  const harness = makeHarness();

  return Effect.gen(function* () {
    const adapter = yield* OpenCodeAdapter;
    const threadId = ThreadId.make("thread-opencode-pending-request-stop");
    ...
    harness.events.push({
      ...
      properties: {
        ...
        tool: { messageID: "msg_stop_permission", callID: "call_stop_permission" },
      },
    });
    harness.events.push({
      type: "question.asked",
      properties: {
        ...
        tool: { messageID: "msg_stop_question", callID: "call_stop_question" },
      },
    });
    yield* drainFibers;
    ...
    const resolvedIndex = observed.findIndex(
      (event) => event.type === "request.resolved" && event.requestId === permissionId,
    );
    const exitedIndex = observed.findIndex((event) => event.type === "session.exited");
    NodeAssert.equal(resolvedIndex >= 0, true);
    NodeAssert.equal(exitedIndex > resolvedIndex, true);
